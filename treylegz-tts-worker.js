// ===================================================================
//  READ IT TO ME  ·  edge voice synthesizer
  ...
  try {
    ...
    if (d && d.voices) {
      for (let i = 0; i < d.voices.length; i++) {
        ...
        const nm = (d.voices[i].name || "").trim();
        if (!nm || nm.toLowerCase() === "untitled") continue; // skip blank / "Untitled" voices
        list.push({ id: vid, name: nm });
      }
    }
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .head{width:100%;max-width:680px;margin:8px 0 22px}
  .tagline{font-family:var(--tech);font-weight:600;font-size:clamp(11px,2.7vw,13px);letter-spacing:.17em;text-transform:uppercase;line-height:1.7;color:var(--muted);text-align:center;max-width:490px;margin:18px auto 0;padding:0 6px}
  ...
  .reel{
    ...
    width:clamp(22px,6.8vw,46px);height:clamp(38px,11vw,68px);background:#050409;
  ...
  .glyph{
    height:clamp(38px,11vw,68px);display:flex;align-items:center;justify-content:center;
    font-family:var(--disp);font-weight:900;font-size:clamp(20px,6.2vw,42px);line-height:1;letter-spacing:.01em;
...
<body>
  <header class="head">
    <div class="slot" id="slot" aria-label="READ IT TO ME"></div>
    <p class="tagline">Enter words you would like to have read, then select a voice and finally, press <span>Read it</span></p>
  </header>
